...
icd_chapters <-
  tibble(
    node = xml_find_all(load_xml, ".//chapter"),
    name = xml_text(xml_find_first(node, ".//name")),
    desc = xml_text(xml_find_first(node, ".//desc"))
  ) %>% 
  select(-node)
...
icd_section <-
  tibble(
    node = xml_find_all(load_xml, ".//section"),
    id = xml_attr(node, "id"),
    desc = xml_text(xml_find_first(node, ".//desc"))
  ) %>% 
  select(-node)
...
# dx ----
dx_nodes <- xml_find_all(load_xml, ".//diag")

dx_long <- 
  tibble(
    code = xml_find_first(dx_nodes, ".//name") %>% xml_text(),
    text = xml_find_first(dx_nodes, ".//desc") %>% xml_text(),
    length = nchar(code),
    one_less = str_sub(code, 1, -2)
  )

join_dx <- function(df, n) {
  old = paste0("first_", n)
  new = paste0("first_", n + 1)
  new_text = paste0("first_", n + 1, "_text")
  # cat(
  #   old,
  #   new,
  #   new_text
...
a <- 
  tibble(
    name = xml_find_first(dx_nodes, ".//name") %>% xml_text(),
    desc = xml_find_first(dx_nodes, ".//desc") %>% xml_text(),
    sevenChrNote = xml_find_first(dx_nodes, ".//note") %>% xml_text()
    # sevenChrDef = as.list(xml_find_all(dx_nodes, ".//extension")) #, char = as.list(xml_attr(xml_find_first(dx_nodes, ".//extension"), "char"))
  )


note = xml_find_all(load_xml, ".//sevenChrNote/note") %>% xml_text(),
# node_n = xml_find_all(load_xml, ".//diag//sevenChrNote"),
ext = xml_find_all(load_xml, ".//sevenChrDef/extension") %>% as.list(xml_text())
) 
%>% 
  ...
  icd_extensions <-
    data_frame(
      node = xml_find_all(load_xml, ".//extension"),
      char = node %>% xml_attr("char"),
      text = node %>% xml_text()
      #xml_attrs("first")#,
      #desc = xml_text(xml_find_first(.x, ".//desc"))
    )
  ...
  icd_extensions <-
    dx_nodes %>% 
    mutate(
      char_n = node_n %>% xml_text(),
      char = node_e %>% as.list(xml_attr(node_e, "char")),
      text_e = node_e %>% xml_text()
      #xml_attrs("first")#,
      #desc = xml_text(xml_find_first(.x, ".//desc"))
    )
  ...
  raw_dx <-
    ...
    slice(1:5500) %>% 
    mutate(
      name = xml_find_first(node, ".//name") %>% xml_text(),
      desc = xml_find_first(node, ".//desc") %>% xml_text(),
      sevenChrNote = xml_find_first(node, ".//note") %>% xml_text(),
      sevenChrDef = xml_find_first(node, ".//extension") %>% as.list(xml_text())
    ) %>% 
    select(-node)
  ...
  icd_ext <-
    ...
    mutate(
      #char = xml_find_all(node, ".//extension") %>% as.list(xml_text()),
