...
get_categories_high <-
  ...
  mutate(level_1 = gsub(". .*", "", description),
...
```{r}
data_cdc <- 
  read.csv("icd10_codes_cdc.csv", stringsAsFactors = F)
  #mutate(code_3 = str_sub(original, 1, 3), code = paste0(code_3, ".", str_sub(original, 4)))
...
step_1 <-
  ...
  mutate(search_base = str_replace_all(description2, paste0("\\b(", remove_words, ")\\b"), "")) %>% 
  mutate(search_base = str_replace_all(search_base, "\\s+", " ")) %>% 
...
c5 <- 
  ...
  ungroup() %>% 
  mutate(c5_lookup = str_replace_all(c5_phrase, "\\s", "|"),
         c5_lookup = paste0("\\b(", c5_lookup, ")\\b")) %>%
  distinct(code_5, c5_phrase, c5_lookup)

c6 <-
  each_word %>% 
  #filter(str_detect(code_3, "E83.0")) %>% #E83 is failing
  ...
  ungroup() %>% 
  mutate(c6_lookup = str_replace_all(c6_phrase, "\\s", "|"),
...
c8 <-
  ...
  ungroup() %>%
  mutate(c8_lookup = str_replace_all(c8_phrase, "\\s", "|"),
...
c7 <-
  ...
  ungroup() %>% 
  mutate(c7_lookup = str_replace_all(c7_phrase, "\\s", "|"),
...
filter(results, str_detect(code, "S42.2")) %>% select(ends_with("phrase")) %>% head
 

write.csv(results, "first_pass.csv", row.names = F, na = ".")
```
...
step_2 <-
  step_1 %>% 
  group_by(code_5, D4) %>% 
    mutate(search_base2 = lag(search_base, default = lead(search_base)),
           search_base2 = gsub("\\s", "|", search_base2)) %>% 
    ungroup() %>% 
    rowwise() %>% 
    mutate(different_8 = gsub(search_base2, "", search_base),
...
%>% 
  group_by(code_5) %>% 
    ...
    ungroup() %>% 
    rowwise() %>% 
    mutate(different_5 = gsub(search_base2, "", search_base),
